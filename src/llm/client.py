...
class CodexCLISettings:
    ...
    def from_env() -> "CodexCLISettings":
        ...
        model = os.environ.get("AGENT_FLOW_CODEX_MODEL")
        profile = os.environ.get("AGENT_FLOW_CODEX_PROFILE")
        sandbox = os.environ.get("AGENT_FLOW_CODEX_SANDBOX", "read-only")
        ask_for_approval = os.environ.get("AGENT_FLOW_CODEX_APPROVAL", "never")
        search = os.environ.get("AGENT_FLOW_CODEX_SEARCH", "").strip().lower() in {"1", "true", "yes", "on"}
        timeout_raw = os.environ.get("AGENT_FLOW_CODEX_TIMEOUT_S", "900")
        try:
            timeout_s = int(timeout_raw)
        except ValueError:
            timeout_s = 900
        config = _load_json_env("AGENT_FLOW_CODEX_CONFIG_JSON") or {}
