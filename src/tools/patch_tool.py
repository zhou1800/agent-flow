...
class PatchTool:
    ...
    def apply(self, patch_text: str) -> ToolResult:
        ...
        if not shutil.which("git"):
            return ToolResult(ok=False, summary="git not available", data={}, elapsed_ms=_elapsed_ms(start), error="git is required")
        try:
            ...
            return ToolResult(ok=True, summary="patch applied", data={}, elapsed_ms=_elapsed_ms(start))
        except Exception as exc:
            return ToolResult(ok=False, summary="patch error", data={}, elapsed_ms=_elapsed_ms(start), error=str(exc))
